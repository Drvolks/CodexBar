...
struct MenuBarPane: View {
    ...
    private var mergedIconMetricsRow: some View {
        LabeledContent {
            ...
            SettingsRowLabel(L("merged_icon_metrics_title"), subtitle: self.mergedIconMetricsSubtitle)
        }
    }

    private var mergedIconMetricsSubtitle: String {
        if !self.settings.mergeIcons {
            L("merged_icon_metrics_enable_merge_icons_hint")
        } else if self.activeProvidersInOrder.isEmpty {
            L("merged_icon_metrics_no_providers_hint")
        } else {
            self.mergedIconMetricsSelectionSummary
        }
    }
    ...
    private var mergedIconMetricsPopover: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text(L("merged_icon_metrics_choose_providers"))
                .font(.headline)
            Text(L("merged_icon_metrics_hint", String(Self.maxIconMetricProviders)))
                .font(.footnote)
                .foregroundStyle(.tertiary)
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ForEach(self.activeProvidersInOrder, id: \.self) { provider in
                        ...
                        HStack(alignment: .center, spacing: 8) {
                            ...
                            .disabled(
                                !isSelected &&
                                    self.mergedIconMetricProviders.count >= Self.maxIconMetricProviders)
    ...
    private func metricOptions(for provider: UsageProvider) -> [MenuBarPaneMetricOption] {
        ...
        if self.settings.menuBarMetricSupportsTertiary(for: provider, snapshot: snapshot) {
            ...
            options.append(MenuBarPaneMetricOption(
...
private struct MenuBarPaneMetricOption: Identifiable {
    let id: String
    let title: String
}
