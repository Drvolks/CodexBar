...
extension StatusItemController {
    static let loadingPercentEpsilon = 0.0001
        ...
        bypassMergedMenuTrackingDeferral: Bool = false) -> Bool
    {
        ...
        if let handled = self.applyMergedMenuBarMetricIconIfNeeded(
            phase: phase, button: button, needsAnimation: needsAnimation) { return handled }
    ...
    func primaryProviderForUnifiedIcon() -> UsageProvider {
        ...
        for provider in self.store.enabledFirstPartyProviders() where mergedProviderSet.contains(provider) {
        ...
        if let enabled = self.store.enabledFirstPartyProviders().first(where: { mergedProviderSet.contains($0) }) {
            return enabled
        }
